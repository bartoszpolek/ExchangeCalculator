...
class AmountInputNormalizerTest {
    ...
    @Test fun `empty input normalizes to zero`() {
        assertThat("".normalizeAmountInput()).isEqualTo("0")
    }

    @Test fun `decimal separators are normalized and fraction is limited`() {
    ...
    @Test fun `leading decimal separator gets zero prefix`() {
        assertThat(".50".normalizeAmountInput()).isEqualTo("0.50")
    }

    @Test fun `leading zeros are collapsed`() {
        assertThat("00012.30".normalizeAmountInput()).isEqualTo("12.30")
    }

    @Test fun `trailing decimal separator is preserved while editing and removed on finalize`() {
        assertThat("44.".normalizeAmountInput()).isEqualTo("44.")
        assertThat("44.".finalizeAmountInput()).isEqualTo("44")
    }

    @Test fun `finalize trims redundant fraction zeros`() {
    ...
    @Test fun `first typed digit replaces initial zero even when inserted before zero`() {
        assertThat("10".normalizeAmountInput(previousInput = "0")).isEqualTo("1")
    }

    @Test fun `non amount characters are ignored`() {
        assertThat("$1a2 3".normalizeAmountInput()).isEqualTo("123")
    }

    @Test fun `ignored leading zero insertion keeps previous cursor position`() {
    ...
    @Test fun `duplicate decimal separator keeps previous cursor position`() {
        val previousValue = TextFieldValue(
            text = "12.34",
            selection = TextRange(1),
        )
    ...
    @Test fun `digit inserted before first fraction digit is rejected when fraction is full`() {
        val previousValue = TextFieldValue(
            text = "12.33",
    ...
    @Test fun `digit inserted before only fraction digit is accepted`() {
        val previousValue = TextFieldValue(
            text = "1.6",
            selection = TextRange(2),
        )
    ...
    @Test fun `fraction digit can be appended after existing fraction digit`() {
        val proposedValue = TextFieldValue(
            text = "12.34",
