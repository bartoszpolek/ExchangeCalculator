...
class ExchangeRatesRepositoryImplTest {
    ...
    private val repository = ExchangeRatesRepositoryImpl(remoteDataSource)

    @Test fun `returns available rate when remote succeeds`() = runTest {
    ...
    @Test fun `returns unavailable when remote returns null entry`() = runTest {
        val mxn = CurrencyCode("MXN")
    ...
    @Test fun `returns unavailable when remote response has no matching rate`() = runTest {
        val mxn = CurrencyCode("MXN")
    ...
    @Test fun `server failure maps to unavailable`() = runTest {
        val mxn = CurrencyCode("MXN")
    ...
    @Test fun `client failure maps to unavailable`() = runTest {
        val mxn = CurrencyCode("MXN")
    ...
    @Test fun `unknown failure maps to unavailable`() = runTest {
        val mxn = CurrencyCode("MXN")
        remoteDataSource.nextResult = Result.Failure(
    ...
    @Test fun `connection failure maps to network failure`() = runTest {
        val mxn = CurrencyCode("MXN")
