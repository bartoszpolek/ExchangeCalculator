...
class ExchangeViewModelTest {
    ...
    @Test
    fun `loads currencies in init and fetches first currency rate`() = runTest {
        ...
        viewModel.state.test {
            ...
            assertThat(state.rateState.availableFormattedRate()).isEqualTo("18.4087")
    ...
    @Test
    fun `empty currencies leave screen in loading and do not fetch rate`() = runTest {
        val ratesRepository = FakeExchangeRatesRepository(
            rateResults = mutableMapOf(
                MXN to RateFetchResult.Available(exchangeRate(MXN)),
            ),
        ...
        val viewModel = createViewModel(
            currencyRepository = FakeCurrencyListRepository(currencies = emptyList()),
            ratesRepository = ratesRepository,
        )

        assertThat(viewModel.state.value).isEqualTo(ExchangeUiState.Loading)
        assertThat(ratesRepository.requestedCurrencies).containsExactly()
    }

    @Test
    fun `typing USDC amount converts fiat amount`() = runTest {
    ...
    @Test
    fun `converted whole amount omits redundant zero fraction`() = runTest {
        val viewModel = createReadyViewModel()
    ...
    @Test
    fun `typing fiat amount converts USDC amount`() = runTest {
        val viewModel = createReadyViewModel()
    ...
    @Test
    fun `empty active amount clears converted amount`() = runTest {
        val viewModel = createReadyViewModel()
        viewModel.onAction(
            ExchangeAction.OnAmountChange(
                input = ExchangeInput.USDC,
    ...
    @Test
    fun `swap toggles input positions and makes new top input active`() = runTest {
        val viewModel = createReadyViewModel()
        viewModel.onAction(
            ExchangeAction.OnAmountChange(
                input = ExchangeInput.USDC,
    ...
    @Test
    fun `refresh after swap keeps top fiat amount and recalculates bottom USDC amount`() = runTest {
        val ratesRepository = FakeExchangeRatesRepository(
            rateResults = mutableMapOf(
    ...
    @Test
    fun `selecting currency fetches new rate and recalculates current amount`() = runTest {
        val ratesRepository = FakeExchangeRatesRepository(
            rateResults = mutableMapOf(
                MXN to RateFetchResult.Available(exchangeRate(MXN, ask = "2", bid = "2")),
    ...
    @Test
    fun `selecting current currency dismisses sheet without fetching rate again`() = runTest {
        val ratesRepository = FakeExchangeRatesRepository(
            rateResults = mutableMapOf(
                MXN to RateFetchResult.Available(exchangeRate(MXN, ask = "2", bid = "2")),
            ),
    ...
    @Test
    fun `unavailable selected rate clears converted amount`() = runTest {
        val ratesRepository = FakeExchangeRatesRepository(
            rateResults = mutableMapOf(
                MXN to RateFetchResult.Available(exchangeRate(MXN, ask = "2", bid = "2")),
    ...
    @Test
    fun `network failure stores rate failure and refresh fetches selected currency again`() =
        runTest {
            val ratesRepository = FakeExchangeRatesRepository(
                rateResults = mutableMapOf(
                    MXN to RateFetchResult.Available(exchangeRate(MXN, ask = "2", bid = "2")),
    ...
    @Test
    fun `refresh network failure keeps current rate and shows connection banner`() = runTest {
        val ratesRepository = FakeExchangeRatesRepository(
            rateResults = mutableMapOf(
                MXN to RateFetchResult.Available(exchangeRate(MXN, ask = "2", bid = "2")),
            ),
    ...
    private fun ExchangeRateUiState.availableFormattedRate(): String =
        (this as ExchangeRateUiState.Available).formattedRate
